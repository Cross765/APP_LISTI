...
scroll_secciones = ctk.CTkScrollableFrame(sidebar, width=220, fg_color="transparent", scrollbar_button_color="#DADADA", scrollbar_button_hover_color="#CCCCCC")
...
def cargar_notas():
    ...
    for idx, (nid, titulo) in enumerate(notas):
        ...
        btn_nota = ctk.CTkButton( frame, text=titulo, text_color="#333333", font=ctk.CTkFont(size=14), fg_color="transparent", hover_color="#E0D4EF", anchor="w", command=lambda n=nid: mostrar_nota(n))
            ...
            def mover():
                ...
                try:
                    ...
                    def confirmar():
                        ...
                        db.mover_nota(nota_id, ids[elegido])
        ...
        btn_menu = ctk.CTkButton(frame, text="⋮", width=25, fg_color="transparent", text_color="#666666", hover_color="#D1B6F1", command=lambda n=nid, p=frame: abrir_menu(n, p))
...
def nueva_nota():
    ...
    # Título
    titulo = ctk.CTkLabel(frame_editor, text="📒 Nueva Nota", font=ctk.CTkFont(size=18, weight="bold"), text_color="#4A148C")
    titulo.pack(pady=(10, 8))

    # Entrada de texto
    entrada = ctk.CTkEntry(frame_editor, width=400, placeholder_text="Escribe el título de tu nota...")
    ...
    btn_crear = ctk.CTkButton(botones, text="✔ Crear", fg_color="#B99AD9", text_color="white",corner_radius=8, width=100)
    btn_crear.pack(side="left", padx=10)

    btn_cancelar = ctk.CTkButton(botones, text="✖ Cancelar", fg_color="#F44336", text_color="white", corner_radius=8, width=100)
...
def mostrar_nota(nota_id):
    ...
    frame_editor.grid(row=0, column=0, sticky="nsew", padx=1, pady=1)
...
def nueva_carpeta():
    ...
    ctk.CTkButton(botones, text="Crear", command=guardar_nombre, fg_color="#B99AD9", text_color="white").pack(side="left", padx=5)
    ctk.CTkButton(botones, text="Cancelar", command=lambda: frame_editor.destroy(), fg_color="#F44336", text_color="white").pack(side="left", padx=5)
...
def mostrar_carpeta(carpeta_id):
    ...
    nombre, fecha_creacion = row

    titulo = ctk.CTkLabel(frame_editor, text=f"📂 {nombre}",
    ...
    try:
        ...
        if not notas:
            ctk.CTkLabel(frame_editor, text="(Carpeta vacía)", text_color="#888888").pack(pady=10)
            return

        for idx, (nid, titulo_nota) in enumerate(notas):
            color = colores_secciones[idx % len(colores_secciones)]
            frame = ctk.CTkFrame(frame_editor, fg_color="transparent")
...
sidebar_derecho = ctk.CTkScrollableFrame(ventana, width=250, corner_radius=0, fg_color="white")
...
# Carpeta donde están los sonidos
...
def reproducir_sonido(nombre):
    ...
    if archivo and os.path.isfile(archivo):
        ...
        messagebox.showerror("Error", f"No se encontró el archivo de sonido: {archivo}")
...
def agregar_sonido(nombre, color_fondo, color_icono):
    ...
    ctk.CTkButton(frame, text="▶", width=20, height=20, fg_color="transparent", text_color=color_icono, hover_color="#EEEEEE", command=lambda n=nombre: reproducir_sonido(n)).pack(side="right", padx=10)
    ctk.CTkButton(frame, text="⏹", width=20, height=20, fg_color="transparent", text_color="#F44336", hover_color="#EEEEEE", command=detener_sonido).pack(side="right")
