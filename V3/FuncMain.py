...
def guardar_sesion(usuario):
    with open(sesion_archivo, "w") as f:
        json.dump({"usuario": usuario}, f)

def cargar_sesion():
    if os.path.exists(sesion_archivo):
        with open(sesion_archivo, "r") as f:
            return json.load(f).get("usuario")
    return None

def login(nombre, contraseña, callback_exito, recordar=False):
    nombre = (nombre or "").strip()
    contraseña = (contraseña or "").strip()
