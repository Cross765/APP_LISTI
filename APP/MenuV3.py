...
from Funciones import cargar_icono
import Funciones as db
...
img_titulo = cargar_icono(os.path.join(base_dir, "Imgs_menu", "Imagenes", "titulo.png"))
if img_titulo:
...
def cargar_notas():
    ...
    for idx, (nid, titulo) in enumerate(notas):
        ...
        btn_nota.pack(side="left", pady=2)

        notas_widgets[nid] = (frame, btn_nota)
...
def mostrar_nota(nota_id):
    ...
    ctk.CTkButton(botones_frame, text="Guardar", fg_color="#B99AD9", text_color="white",
                  command=guardar_contenido).pack(side="left", padx=5)
    ctk.CTkButton(botones_frame, text="Eliminar", fg_color="#F44336", text_color="white",
                  command=eliminar_nota).pack(side="left", padx=5)
    ctk.CTkButton(botones_frame, text="Cerrar", fg_color="#9E9E9E", text_color="white",
                  command=cerrar_editor).pack(side="left", padx=5)
...
# calendario
titulo_calendario = ctk.CTkLabel(sidebar_derecho, text="Calendario",
                                 font=ctk.CTkFont(size=20, weight="bold", family="Comic Sans MS"),
                                 text_color="#333333")
...
def dibujar_calendario(frame, mes, anio):
    ...
    ctk.CTkButton(encabezado, text="←", width=20, height=20, fg_color="transparent",
                  text_color="#B99AD9", hover_color="#E0D4EF",
                  command=lambda: cambiar_mes(-1)).pack(side="left")

    nombre_mes = datetime(anio, mes, 1).strftime("%B %Y").capitalize()
    ctk.CTkLabel(encabezado, text=nombre_mes,
                 font=ctk.CTkFont(size=14, weight="bold"),
                 text_color="#333333").pack(side="left", padx=6)
    ...
    for i, dia in enumerate(dias_semana):
        ctk.CTkLabel(frame, text=dia, width=20, height=20, text_color="#666666",
                     font=ctk.CTkFont(size=10)).grid(row=1, column=i, padx=1, pady=1)
...
# temporizador (UI)
titulo_temporizador = ctk.CTkLabel(sidebar_derecho, text="Temporizador",
...
ctk.CTkLabel(temporizador_frame, text="00:00",
             font=ctk.CTkFont(size=34, weight="bold"),
             text_color="#A3B044").pack(pady=(8, 0))

ctk.CTkLabel(temporizador_frame, text="¡Hora de enfocarse!",
             font=ctk.CTkFont(size=12), text_color="#333333").pack(pady=(0, 5))
...
# sonidos (UI)
titulo_sonidos = ctk.CTkLabel(sidebar_derecho, text="Sonidos Relajantes",
                               font=ctk.CTkFont(size=16, weight="bold", family="Comic Sans MS"),
                               text_color="#333333")
...
scroll_sonidos = ctk.CTkScrollableFrame(contenedor_sonidos, width=220, fg_color="transparent",
                                        scrollbar_button_color="#DADADA", scrollbar_button_hover_color="#CCCCCC")
scroll_sonidos.pack()
...
def agregar_sonido(nombre, color_fondo, color_icono):
    ...
    ctk.CTkButton(frame, text="▶", width=20, height=20, fg_color="transparent",
                  text_color=color_icono, hover_color="#EEEEEE").pack(side="right", padx=10)
...
# BOTONES ENCABEZADO
iconos = {
    "Tablero": cargar_icono(os.path.join(base_dir,"Imgs_menu","diseño_base", "tablero.png")),
    "Mis Notas": cargar_icono(os.path.join(base_dir,"Imgs_menu","diseño_base", "mis_notas.png")),
    "Carpetas": cargar_icono(os.path.join(base_dir,"Imgs_menu","diseño_base", "carpeta.png")),
    "Papelera": cargar_icono(os.path.join(base_dir,"Imgs_menu","diseño_base", "papelera.png"))
}
