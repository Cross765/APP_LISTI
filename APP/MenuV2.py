...
def agregar_seccion(nombre=None, color=None):
    global contador_secciones
    
    if not nombre:
        contador_secciones += 1
        nombre = f"Nueva sección {contador_secciones}"
    
    if not color:
        color = colores_secciones[(contador_secciones - 1) % len(colores_secciones)]
    ...
    lbl_nombre = ctk.CTkLabel(frame, text=nombre, text_color="#333333", font=ctk.CTkFont(size=14))
    lbl_nombre.pack(side="left", pady=5)

    secciones.append((nombre, frame))

btn_agregar_seccion = ctk.CTkButton(
    sidebar,
    ...
    command=lambda: agregar_seccion()
)
btn_agregar_seccion.pack(fill="x", pady=10, padx=5)

# Sección inicial por defecto
agregar_seccion("Notas rápidas", "#FFC107")
...
def dibujar_calendario(frame, mes, anio):
    ...
    btn_der = ctk.CTkButton(encabezado, text="→", width=20, height=20, fg_color="transparent", text_color="#B99AD9", hover_color="#E0D4EF", command=lambda: cambiar_mes(1))
