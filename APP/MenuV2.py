...
def cargar_notas():
    ...
    for idx, (nid, titulo) in enumerate(notas):
        ...
        def abrir_menu(nota_id, parent):
            ...
            def mover():
                menu.destroy()
                messagebox.showinfo("Mover", f"Función mover nota {nota_id} (pendiente)")

            # opciones del menú
            ctk.CTkButton(menu, text="✏️ Modificar", fg_color="transparent", text_color="#333333",
                          anchor="w", command=modificar).pack(fill="x", padx=5, pady=2)
            ctk.CTkButton(menu, text="🗑️ Borrar", fg_color="transparent", text_color="#333333",
                          anchor="w", command=borrar).pack(fill="x", padx=5, pady=2)
            ctk.CTkButton(menu, text="📂 Mover", fg_color="transparent", text_color="#333333",
                          anchor="w", command=mover).pack(fill="x", padx=5, pady=2)
